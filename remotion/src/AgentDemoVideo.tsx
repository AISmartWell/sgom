...
const Chrome: React.FC<{ step: number; title: string }> = ({ step, title }) => {
  ...
  return <div style={{ position: "absolute", top: 60, left: 110, right: 110, display: "flex", justifyContent: "space-between", alignItems: "center", opacity: o, fontFamily: font }}>
    <div style={{ display: "flex", alignItems: "center", gap: 22 }}>
      <div style={{ fontFamily: mono, color: C.em, fontSize: 22, letterSpacing: 4 }}>STEP {step} / 3</div>
      <div style={{ color: C.fg, fontSize: 44, fontWeight: 700 }}>{title}</div>
    </div>
    <div style={{ fontFamily: mono, fontSize: 18, color: C.amber, border: `1px solid ${C.amber}88`, padding: "8px 14px", letterSpacing: 2 }}>ILLUSTRATIVE DEMO · BRAWNER 10-15</div>
  </div>;
};
...
const Intro: React.FC = () => {
  ...
  return <AbsoluteFill style={{ fontFamily: font, padding: "0 140px", justifyContent: "center" }}>
    <div style={{ fontFamily: mono, color: C.em, fontSize: 26, letterSpacing: 8, opacity: s }}>SGOM · GEOPHYSICAL AI AGENT</div>
    <div style={{ color: C.fg, fontSize: 120, fontWeight: 700, lineHeight: 1.02, marginTop: 24, transform: `translateY(${(1 - s) * 40}px)`, opacity: s }}>From raw well data<br /><span style={{ color: C.cy }}>to a decision.</span></div>
    <div style={{ color: C.mute, fontSize: 34, marginTop: 36, opacity: interpolate(f, [25, 45], [0, 1], { extrapolateRight: "clamp" }) }}>Watch the agent work — upload, compute, result.</div>
  </AbsoluteFill>;
};

const files = [
  { n: "BRAWNER_10-15_logs.las", k: "LAS · GR, SP, RES, NPHI, RHOB", s: "2.4 MB" },
  { n: "completion_report_1978.pdf", k: "Scanned PDF · perforations", s: "5.1 MB" },
  { n: "production_history.csv", k: "Monthly oil / water / gas", s: "84 KB" },
  { n: "core_analysis_1979.pdf", k: "Porosity · permeability", s: "3.7 MB" },
];
const Upload: React.FC = () => {
  const f = useCurrentFrame();
  return <AbsoluteFill style={{ fontFamily: font }}>
    <Chrome step={1} title="Data uploaded" />
      ...
      {files.map((x, i) => {
        ...
        return <div key={x.n} style={{ background: C.panel, border: `1px solid ${p >= 1 ? C.em + "88" : C.line}`, padding: 34, opacity: o, transform: `translateY(${(1 - o) * 30}px)` }}>
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            ...
            <div style={{ color: p >= 1 ? C.em : C.mute, fontSize: 24, fontFamily: mono }}>{p >= 1 ? "✓ PARSED" : `${Math.round(p * 100)}%`}</div>
          </div>
    ...
    <div style={{ position: "absolute", bottom: 110, left: 110, color: C.fg, fontSize: 36, opacity: interpolate(f, [110, 130], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" }) }}>
      <span style={{ color: C.em }}>●</span> 4 sources · 375 ft of log · 38 years of production → ready for analysis
    </div>
  </AbsoluteFill>;
};

const logLines = [
  "Reading curves GR · SP · RES · NPHI · RHOB",
  "Depth-matching core samples to log (4,850–5,225 ft)",
  "Shale volume from gamma ray",
  "Porosity from neutron–density crossplot",
  "Water saturation · resistivity model",
  "Permeability estimate from porosity & Swirr",
  "Cross-check vs perforation record",
  "Flag unperforated intervals with pay response",
];
...
const Compute: React.FC = () => {
  ...
  return <AbsoluteFill style={{ fontFamily: font }}>
    <Chrome step={2} title="Agent computes" />
    ...
    <div style={{ position: "absolute", top: 190, left: 1060, right: 110, background: C.panel, border: `1px solid ${C.line}`, padding: 36, height: 780, boxSizing: "border-box" }}>
      <div style={{ fontFamily: mono, color: C.cy, fontSize: 22, letterSpacing: 3, marginBottom: 26 }}>AGENT REASONING</div>
      {logLines.map((l, i) => {
        const st = 15 + i * 28;
      ...
      <div style={{ position: "absolute", bottom: 36, left: 36, right: 36, fontFamily: mono, fontSize: 22, color: C.mute }}>
        Depth scanned: <span style={{ color: C.fg }}>{Math.round(4850 + prog * 375).toLocaleString()} ft</span>
...
const Results: React.FC = () => {
  ...
  const items = [
    { k: "Pay intervals found", v: "3", c: C.em },
    { k: "Unperforated candidates", v: "2", c: C.red },
    { k: "Recommended method", v: "SPT", c: C.cy },
    { k: "Agent confidence", v: "Medium", c: C.amber },
  ];
  return <AbsoluteFill style={{ fontFamily: font }}>
    <Chrome step={3} title="Results" />
    ...
    <div style={{ position: "absolute", top: 190, left: 1060, right: 110, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
      {items.map((x, i) => <div key={x.k} style={{ background: C.panel, border: `1px solid ${C.line}`, borderTop: `3px solid ${x.c}`, padding: 30, opacity: card(i), transform: `translateY(${(1 - card(i)) * 30}px)` }}>
        ...
        <div style={{ color: x.c, fontSize: 64, fontWeight: 700, marginTop: 8 }}>{x.v}</div>
      </div>)}
    </div>
    <div style={{ position: "absolute", top: 640, left: 1060, right: 110, background: C.panel, border: `1px solid ${C.em}66`, padding: 32, opacity: card(5) }}>
      <div style={{ fontFamily: mono, color: C.em, fontSize: 20, letterSpacing: 3 }}>AGENT CONCLUSION</div>
      <div style={{ color: C.fg, fontSize: 28, lineHeight: 1.4, marginTop: 14 }}>Two bypassed intervals show pay response but were never perforated. Candidate for Slot Perforation Technology — verify before field work.</div>
    </div>
  </AbsoluteFill>;
};
...
const Close: React.FC = () => {
  ...
  return <AbsoluteFill style={{ fontFamily: font, justifyContent: "center", alignItems: "flex-start", padding: "0 140px", opacity: o }}>
    ...
    <div style={{ color: C.cy, fontSize: 44, marginTop: 10 }}>Upload data. The agent does the analysis.</div>
    <div style={{ color: C.mute, fontSize: 24, marginTop: 50, fontFamily: mono }}>AI Smart Well Inc. · Illustrative demonstration, not a measured result</div>
  </AbsoluteFill>;
};
