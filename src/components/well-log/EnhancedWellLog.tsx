...
const EnhancedWellLog = ({ wellId, wellName, formation, defaultExpanded = true, totalDepth, showInterpretationByDefault = false }: EnhancedWellLogProps) => {
  ...
  // Depth ticks
  const depthStep = visibleSpan > 2000 ? 500 : visibleSpan > 800 ? 200 : visibleSpan > 300 ? 100 : visibleSpan > 100 ? 50 : visibleSpan > 40 ? 10 : visibleSpan > 20 ? 5 : visibleSpan > 8 ? 2 : 0.5;
        ...
        <div ref={containerRef} className="p-2 space-y-2">
          ...
          <div className="rounded-lg border border-border/30 overflow-hidden" style={{ background: C.bg }} onWheel={handleWheel}>
            <svg ref={svgRef} viewBox={`0 0 ${TOTAL_W} ${totalH}`} className="w-full cursor-crosshair select-none"
              ...
              {/* ═══ GR ZONE COLOR BANDS (API Standard: 45/75) ═══ */}
              {/* Green: Clean Sand / Reservoir (GR ≤ 45 API) */}
              <rect x={GR_X} y={HEADER_H} width={(45 / 150) * GR_W} height={plotH}
                fill="#22c55e" opacity={0.06} />
              {/* Yellow: Silty Sand / Transition (45 < GR ≤ 75) */}
              ...
              {/* Zone labels at top */}
              <text x={GR_X + (22.5 / 150) * GR_W} y={HEADER_H + 10} textAnchor="middle" fill="#22c55e" fontSize="6" fontWeight="600" opacity={0.7}>CLEAN SAND</text>
              <text x={GR_X + (60 / 150) * GR_W} y={HEADER_H + 10} textAnchor="middle" fill="#eab308" fontSize="6" fontWeight="600" opacity={0.7}>SILTY</text>
