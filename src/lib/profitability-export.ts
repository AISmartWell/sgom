...
const TITLE = "SGOM + Maxxwell Production — Combined Profitability Model";
...
export async function exportModelPdf(opts: {
  ...
  const header = (sub: string) => {
    ...
    doc.text("SGOM + Maxxwell Production", M, 24);
  ...
  doc.text(
    `Generated by SGOM · ${new Date().toISOString().slice(0, 10)} · figures in USD; base case includes Arps decline, royalty, severance tax, D&A and SaaS churn`,
    M, H - 18,
  );
