          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <ul className="list-disc pl-5 space-y-1">
                ...
                <li>NVIDIA Vision — core CV analysis and seismic CV analysis</li>
...
└─────────────────────────────────────────────────────────┘`}
          ...
          <div className="space-y-4">
            {[
               ...
               { icon: "🌐", name: "Seismic Interpretation (Stage 5)", route: "/dashboard/geological-analysis",
                 desc: "3D geological model (Three.js). Seismic sections with CV analysis (NVIDIA Vision · NIM VLM): fault detection, horizon mapping, anomaly identification, fluid contacts. Well logs, cross-sections, AI-generated geological reports. Bypassed Reserves Panel for missed pay detection." },
               ...
               { icon: "💵", name: "Economic Analysis (Stage 7)", route: "/dashboard/economic-analysis",
                 desc: "Full economic evaluation with interactive scenario modeling. Per-well and portfolio metrics: ROI (5-year), payback period, annual gross/net profit. Monte Carlo simulation (baseline + importance-sampled tail estimator). Tornado Chart sensitivity analysis. Cumulative net profit projection (60-month). Net Profit = (Added Production × Oil Price − OPEX/bbl) × 365 × Timeline − Treatment Cost." },
               { icon: "⚡", name: "GPU-Accelerated Monte Carlo (research prototype)", route: "/dashboard/economic-analysis",
                 desc: "Variance-reduced risk estimator inside the Economic Analysis module. Importance sampling concentrates draws in the P10/P90 tails, so tail quantiles stabilise with fewer effective samples than uniform sampling. Interactive controls: sample budget (2⁸–2¹⁴ draws), Oil Price volatility, CAPEX volatility. ROI distribution histogram (baseline vs tail-weighted). Convergence comparison chart. No quantum hardware is used and no quantum speedup is claimed — the estimator runs on CPU workers, with an optional GPU backend for larger portfolios." },
                { icon: "📊", name: "Geophysical Expertise (Stage 8)", route: "/dashboard/geophysical",
                 desc: "9-step well log interpretation (Schlumberger methodology). Lithology (GR: Linear vs Larionov Vshale), Fluid ID (Resistivity ≥10 Ω·m = HC), DEN-NPHI Crossover, Porosity (8% cutoff), Archie Equation (a=1, m=2, n=2, Rw=0.04), Ko Ko Rules (4-curve deflection), Net Pay & Missed Pay (φ_eff>8%, Sw<60%, Vsh<40%). Supports LAS 2.0 import and PDF log digitization. Step 7 (Timur Permeability): k = 0.136·φ⁴·⁴/Swirr² in mD with quality classification (excellent/good/fair/poor/tight); displays input parameter grid with units (mD, frac, Ω·m); automatic input validation with warnings for out-of-range φ, Swirr and Archie constants (a, m, n, Rw); dual-axis depth chart overlaying log₁₀(k) vs Archie Sw with Pearson correlation coefficient (r); export of current visualization and correlation table to PNG (html2canvas), SVG (vector) and CSV (with metadata header: r, n, mean/max k)." },
        ...
        <Section icon={Cpu} title="6. Edge Functions (12 total)">
          <div className="space-y-3">
            {[
              ...
              { name: "analyze-core", desc: "Analyze core images via NVIDIA Vision (NIM VLM). Determine lithology, porosity, texture, mineral composition.", input: "{ imageBase64 }", output: "{ analysis: { lithology, porosity, ... } }" },
              { name: "analyze-core-cv", desc: "Advanced core CV analysis with NVIDIA Vision. Multi-stage pipeline (edge detection, segmentation, classification).", input: "{ imageBase64, rockType? }", output: "{ analysis, model }" },
              ...
              { name: "analyze-seismic-cv", desc: "Computer vision seismic image analysis via NVIDIA Vision. Detects faults, horizons, anomalies, fluid contacts.", input: "{ imageBase64, analysisMode, wellContext? }", output: "{ analysis, model }" },
        ...
        <Section icon={Brain} title="13. Core Thesis — Restoration Potential Score (RPS)">
          ...
          <p><strong>Restoration Potential Score (RPS):</strong> A 0–100 score per well aggregating the 9 pipeline stages. The score is <em>not yet calibrated</em>: no blind validation has been completed. Phase I R&amp;D targets are reported at two levels — <em>subcomponent targets</em> measure a single module in isolation (e.g. ≥75% formation-attribution accuracy, ≤30% curve-reconstruction error on ≥200 masked wells), while <em>overall targets</em> measure the end-to-end hypothesis on blind data (&lt;15% MAPE degradation on unseen regions; ≥20% MAPE reduction against MICE/GAIN baselines). The two are not interchangeable. Uncertainty is scored as empirical coverage within ±3 percentage points of the nominal 90% level — over-coverage counts as miscalibration, not success — with Mondrian (region-conditional) conformal prediction and region-disjoint calibration folds as fallback. Evaluation uses leave-one-case-out with bootstrap confidence intervals on a ≥200-well validation set. Fallback if targets are missed: restrict output to a ranked shortlist with explicit abstention instead of an absolute score.</p>
