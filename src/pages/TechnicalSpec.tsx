          ...
          <div className="space-y-4">
            {[
              ...
              { icon: "💰", name: "Financial Calculator", route: "/dashboard/financial",
               ...
               { icon: "⚙️", name: "SPT Parameters (Stage 7)", route: "/dashboard/spt-parameters",
                    desc: "Cutting program generation for well recompletion via slot-perforation technology (Maxxwell Production, Patent US 8863823). 5-stage pipeline: Well Data Input → Cutting Program Generation → Equipment Specifications → Slot Design Optimization → Service Report. SPT Tool components: hydraulic unit, return unit, perforator with tungsten carbide/corundum/diamond nozzles, compensator, adapter. Surface equipment: workover rig, BOP, water tank, cutting tank with vibrator, blender, manifold, frack-van monitoring center, abrasive filler, frack-pump service (high-pressure pumping unit with crew). Interactive slot geometry controls (length, width, slots/row 2–4, row spacing, nozzle count 2–4) with real-time unrolled casing surface visualization (SVG flat layout: X-axis = casing circumference in inches, Y-axis = perforation depth in ft) showing longitudinal slot pattern. Drainage area formula by nozzle count: 2 nozzles → 13 sq ft/linear ft, 3 nozzles → 18 sq ft/linear ft, 4 nozzles → 23 sq ft/linear ft (per Maxxwell Production data). Design optimization radar chart (flow area, slot density, coverage, structural integrity, flow rate, efficiency). Generates complete service program document with cutting distribution by zone (upper/middle/lower), equipment list, SPT process stages, and estimated flow capacity. Key calculations: Open Area = N_slots × L_slot × W_slot; Flow Capacity = Open Area × 42 × 24 bbl/day. Max slot depth: up to 5 ft. Cutting speed: 50 min/ft (cased hole), 30 min/ft (open hole). Expected inflow increase: 5–10× (based on open area)." },
